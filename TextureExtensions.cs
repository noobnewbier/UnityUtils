...
namespace UnityUtils
{
    ...
    {
        public static void TranslateTexture(this Texture2D tex, int xOffset, int yOffset)
        {
            var w = tex.width;
            var h = tex.height;
            var pixelsCount = w * h;
        ...
        public static void RotateTexture(this Texture2D tex, float angle, Color defaultColor = default)
        {
            ...
            for (var y = 0; y < h; y++)
            {
                ...
                for (var x = 0; x < w; x++)
                {
                    ...
                    if (x2Int >= w || x2Int < 0 || y2Int >= h || y2Int < 0) color = defaultColor;
                    else color = originalPixels[y2Int * w + x2Int];
